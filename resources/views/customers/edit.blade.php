...
@section('content')
      ...
      <div class="right_col" role="main">
            ...
            <div class="row">
              ...
              <div class="col-md-12 col-sm-12  ">
                <div class="x_panel">
                  ...
                  <div class="x_content">
                    <br />
                    <form id="demo-form2"  action="{{route('customers.update', $customer->customer_id)}}" method="post" class="w-100   flex-wrap d-flex form-horizontal form-label-left" novalidate="">
                    @csrf
                    @method('PUT') 
                      
                      <div class="mb-3 col-12 col-md-4">
                        <label for="names" class="form-label">Name</label>
                        <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" value="{{ old('name', $customer->name) }}"  >
                      ...
                      <div class="mb-3 col-12 col-md-4">
                        ...
                        <input type="text" name="contact" class="form-control {{ $errors->has('contact') ? 'is-invalid' : '' }}" value="{{ old('contact', $customer->contact) }}">
                        @error('contact') <span class="text-red-500 text-danger">{{ $message }}</span> @enderror
                      </div>
                      <div class="mb-3 col-12 col-md-4">
                        <label for="addresss" class="form-label"> Address</label>
                        <input type="text" name="address" class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" value="{{ old('address', $customer->address) }}">
                        @error('address') <span class="text-red-500 text-danger">{{ $message }}</span> @enderror
                      </div>
                      <div class="mb-3 col-12 col-md-4">
                        <label  class="form-label">Opening Balance</label>
                        <input type="text" name="op_balance" id="op_balance" class="form-control {{ $errors->has('op_balance') ? 'is-invalid' : '' }}" value="{{ old('op_balance', abs($customer->op_balance)) }}">
        ...
        <select class="form-control" id="transactionType" name="transaction_type">
            ...
            <option value="debit" {{ old('transaction_type', $customer->op_balance > 0 ? 'debit' : '') == 'debit' ? 'selected' : '' }}>Debit</option>
            <option value="credit" {{ old('transaction_type', $customer->op_balance < 0 ? 'credit' : '') == 'credit' ? 'selected' : '' }}>Credit</option>
        </select>
