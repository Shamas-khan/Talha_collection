...
@section('content')
 ...
 <div class="right_col" role="main">
  ...
  <span>Total Amount: {{ $karahiVendor->total_amount ?? 0}} </span> <br>
  <span>Paid Amount: {{ $karahiVendor->paid_amount ?? 0}} </span><br>
  <span>Remaining Amount: {{ $karahiVendor->remaining_amount ?? 0}} </span>
...
$(document).ready(function() {
          ...
          $("#ttable").DataTable({
            dom: "Bfrtip",
                                  ...
                                  pageLength: 50,
              ajax: {
                url: "{{ route('karahivendor.payment', ['id' => $karahiVendor->karai_vendor_id]) }}",
                  type: 'POST',
                  headers: {
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="productionform" action="{{route('karahivendor.paymentstore')}}"  method="post" class="w-100 align-items-center flex-wrap d-flex form-horizontal form-label-left">
                    @csrf
                    <!-- Hidden input to store issue_material_id -->
                    
                    <input type="hidden" name="karai_vendor_id" value="{{ $karahiVendor->karai_vendor_id }}" >
                    <div class="col-12">
